...
1. **Read Flow Config:** Read `.orkestra/flow.yaml`.
   - *Error Handling:* If this file does not exist, STOP and tell the user: "Orkestra is not initialized. Please run `orkestra init` first."
...
- Always read every file under `.orkestra/instructions/global/` first (these are the always-on rules rendered by Orkestra's generic adapter).
- Treat every path in `global_instructions` and `instruction_files` as relative to `.orkestra/instructions/template/` unless already absolute.
